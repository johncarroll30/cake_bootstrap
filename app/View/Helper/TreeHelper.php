...
class TreeHelper extends AppHelper {
	...
	public function renderCategoryTree($array,$slug,$level_count=0,$level_limit,$absolute_path) { 
		
		if (count($array)) { 
			// If we are in the root display all categries and their headings
				if($level_count=="1") { echo "<div class='panel'>"; }
				echo "\n
				<ul class='inline-list'>\n"; 
				foreach ($array as $vals) {
					
					echo "<li title=".$vals['Category']['name']." id=\"".$vals['Category']['id']."\"><a href='/services/".$vals['Category']['tree']."'>".$vals['Category']['name']."</a>";
					if (count($vals['children'])) { 
						$bold = "";
						$level_count++;
						if($level_count<4) {
							$this->renderCategoryTree($vals['children'],$slug,$level_count,$level_limit,$absolute_path); 
						} 
				...
				echo "</ul>\n"; 
				if($level_count=="1") { echo "</div>"; }
		} 
	}

	public function renderCategoryMenu($array,$level_count=0,$level1,$level2,$level3,$level4,$level5,$level6,$level7,$checkbox=false,$category_ids) { 
		//debug($array);
		if (count($array)) { 
				
				foreach ($array as $vals) {
					$class=null;
					if($level1==$vals['Category']['slug']) : $class="active collapse"; endif; 
					//echo "<li class='".$class."' title='".$vals['Category']['name']."' id='".$vals['Category']['id']."'><a href='/services/".$vals['Category']['tree']."' class='more_info'>".$vals['Category']['name']."</a>";

					$checked = false;
					if($category_ids!=null) : 
						if(in_array($vals['Category']['id'],$category_ids)) : $checked = true; endif;
					endif;

					if($checkbox) :
						echo "<li class='".$class."' title='".$vals['Category']['name']."' id='".$vals['Category']['id']."'><label class='more_info' for=".'a'.$vals['Category']['id'].">".$this->Form->input('id.', array('id' => 'a'.$vals['Category']['id'], 'hiddenField' => false, 'value'=>$vals['Category']['id'],  'type' => 'checkbox', 'checked'=>$checked))." ".$vals['Category']['name']."</label>";
					else :
						echo "<li class='".$class."' title='".$vals['Category']['name']."' id='".$vals['Category']['id']."'><span class='more_info'>".$vals['Category']['name']."</span>";
					endif;
					
					if (count($vals['children'])) { 
						$level_count++;
						//if($level_count<4 && $level1==$vals['Category']['slug']) {
						if($level_count<4) {
							echo "\n<ul>\n"; 
							$this->renderCategoryMenu($vals['children'],$level_count,$level1,$level2,$level3,$level4,$level5,$level6,$level7,$checkbox=true,$category_ids); 
							echo "</ul>\n"; 
						} 
						$level_count = 0;
						$checkbox = false;
	...
	public function renderBreadcrumb($paths,$listing=false,$listing_title=null) { 
		...
		if (count($paths)) { 
			foreach ($paths as $key => $treeCategory) {

				@$slug .= "/" . $treeCategory['Category']['slug'] . '';
				if (!$treeCategory['Category']['active']) {
					continue;
				}
				echo '<li>';
				if ($total === $key + 1 && $listing==false) {
					echo h($treeCategory['Category']['name']);
				} else {
					?>
					<a href="/services<?php echo $slug; ?>" ><?php echo $treeCategory['Category']['name']; ?></a>
					<?php
				}
				echo '</li>';

				if ($total === $key + 1 && $listing==true) {
					echo "<li>".$listing_title."</li>";
				}
...
/* /app/View/Helper/TreeHelper.php */
/*App::uses('AppHelper', 'View/Helper');

class TreeHelper extends AppHelper {
	public $helpers = array('Html');
	public function renderMenu($array,$generated_slugs,$root=true,$hasChildren=false) { 
    	//debug($array);
		...
		if (count($array)) { 
			...
			foreach ($array as $vals) { 
				...
				echo "<li id=\"".$vals['Category']['id']."\"><a href='".$slug_build.$vals['Category']['slug']."'>".$vals['Category']['name']."</a>"; 
				if (count($vals['children'])) { 
					$parent_slug = $slug_build.$vals['Category']['slug']; 
					$this->renderMenu($vals['children'],$root=true,$hasChildren=false); 
